...
/* ==========================================================================
   Настоящий перевод через Google Translate (встроенный в тот же Apps Script,
   что уже развёрнут для Google Таблиц — см. AppsScript_DataStore_Backend.gs,
   действие "translate"). Не требует Cloudflare/Anthropic — использует
   GOOGLE_SHEETS_URL, который уже настроен в config.js.

   В отличие от шаблонного generateBothDrafts(), здесь казахский текст —
   настоящий перевод собранного русского поста, а не обёртка вокруг слов
   сотрудника на русском.
   ========================================================================== */
async function generateTranslatedDraft(submission) {
  if (typeof GOOGLE_SHEETS_URL === "undefined" || !GOOGLE_SHEETS_URL) {
    throw new Error("GOOGLE_SHEETS_URL не настроен — см. config.js");
  }

  const ruText = generatePostDraft(submission, "ru");

  const response = await fetch(GOOGLE_SHEETS_URL, {
    method: "POST",
    headers: { "Content-Type": "text/plain;charset=utf-8" },
    body: JSON.stringify({ action: "translate", text: ruText, from: "ru", to: "kk" })
  });

  const data = await response.json();
  if (data.error || !data.translated) {
    throw new Error("Сервер перевода вернул ошибку: " + (data.error || "нет текста"));
  }

  return { ru: ruText, kk: data.translated };
}
