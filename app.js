...
(function () {
  ...
  /* ---------------------------------------------------------------------- */
  /* Автокоррект орфографии (Yandex Speller) — только для русского текста   */
  /* ---------------------------------------------------------------------- */
  async function correctSpellingRu(text) {
    if (!text) return text;
    if (typeof GOOGLE_SHEETS_URL === "undefined" || !GOOGLE_SHEETS_URL) return text;
    try {
      const payload = { action: "spellcheck", text: text };
      if (typeof API_TOKEN !== "undefined" && API_TOKEN) payload.token = API_TOKEN;
      ...
      console.warn("Автокоррект недоступен, использован исходный текст:", e.message);
      return text;
    }
  }
  ...
  function bindWizard() {
    ...
    document.getElementById("step1").addEventListener("submit", async (e) => {
      ...
      let what = data.get("what").trim();
      let cool = data.get("cool").trim();

      // Автокоррект только при русском интерфейсе — иначе рискуем
      // "исправить" казахский текст сотрудника на русский лад
      if (currentLang === "ru") {
        submitBtn.disabled = true;
        submitBtn.textContent = t("spellcheck_running", currentLang);
        [what, cool] = await Promise.all([correctSpellingRu(what), correctSpellingRu(cool)]);
      ...
      try {
        ...
        li.innerHTML = `
          <div class="submission-main">
            <div class="submission-title">${escapeHtml(a.what)}</div>
            <div class="submission-meta">${escapeHtml(a.whereWhen)} · ${escapeHtml(s.id)}</div>
          </div>
          <span class="status-badge status-${escapeHtml(s.status)}">${escapeHtml(t("status_" + s.status, currentLang))}</span>
        `;
        ul.appendChild(li);
      } catch (err) {
        console.error("Не удалось отрисовать заявку", s && s.id, "— пропускаю. Причина:", err, "Сырые данные:", s);
      }
    });
  }
  ...
  async function renderModDashboard() {
    ...
    list.forEach((s, idx) => {
      try {
        ...
        li.innerHTML = `
          <div class="submission-main">
            <div class="submission-title">${escapeHtml(a.what)}</div>
            <div class="submission-meta">${escapeHtml(s.authorName)} · ${escapeHtml(a.whereWhen)} · ${escapeHtml(s.id)}</div>
          </div>
          <span class="status-badge status-${escapeHtml(s.status)}">${liveDot}${escapeHtml(t("status_" + s.status, currentLang))}</span>
        `;
    ...
    emptyNote.hidden = ul.children.length > 0;
  ...
  function bindModDetail() {
    ...
    document.getElementById("modGenerateBtn").addEventListener("click", async () => {
      ...
      const noteEl = document.getElementById("aiSourceNote");

      btn.disabled = true;
      const originalLabel = btn.textContent;
      btn.textContent = t("mod_generating", currentLang);
      noteEl.hidden = true;

      let drafts;
      let source = "template"; // "ai" | "translated" | "template"
      try {
        drafts = await generateAIDraft(submission);
        source = "ai";
      } catch (errAI) {
        try {
          drafts = await generateTranslatedDraft(submission);
          source = "translated";
        } catch (errTranslate) {
          console.warn("Ни ИИ, ни Google Translate недоступны, использован шаблон:", errAI.message, "/", errTranslate.message);
          drafts = generateBothDrafts(submission);
          source = "template";
        }
      }
      ...
      const noteKeyMap = { ai: "mod_ai_used", translated: "mod_translated_used", template: "mod_ai_fallback" };
      noteEl.textContent = t(noteKeyMap[source], currentLang);
      noteEl.className = "ai-source-note " + (source === "template" ? "ai-source-warn" : "ai-source-ok");
      noteEl.hidden = false;
      document.getElementById("postKk").classList.toggle("needs-review", source === "template");
  ...
  async function renderModDetail(id) {
    ...
    document.getElementById("postKk").classList.remove("needs-review");
