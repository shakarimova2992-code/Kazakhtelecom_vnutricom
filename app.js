...
(function () {
  ...
  async function correctSpellingRu(text) {
    if (!text) return text;
    try {
      const url = "https://speller.yandex.net/services/spellservice.json/checkText?options=6&lang=ru&text=" + encodeURIComponent(text);
      ...
      let corrected = text;
      // применяем исправления с конца строки к началу — иначе после первой
      // же замены сместятся позиции последующих найденных ошибок
      ...
      try {
        ...
        li.innerHTML = `
          <div class="submission-main">
            ...
            <div class="submission-meta">${escapeHtml(a.whereWhen)} · ${s.id}</div>
          </div>
    ...
    list.forEach((s, idx) => {
      try {
        ...
        li.innerHTML = `
          <div class="submission-main">
            <div class="submission-title">${escapeHtml(a.what)}</div>
            <div class="submission-meta">${escapeHtml(s.authorName)} · ${escapeHtml(a.whereWhen)} · ${s.id}</div>
          </div>
