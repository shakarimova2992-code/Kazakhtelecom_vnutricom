...
async function sheetsGet_(params) {
  // Добавляем уникальный параметр и отключаем кэш браузера — иначе повторный
  // запрос к тому же ?action=list может вернуть старый закэшированный ответ
  // вместо актуальных данных из таблицы.
  params.set("_ts", Date.now().toString());
  const res = await fetch(`${GOOGLE_SHEETS_URL}?${params.toString()}`, { cache: "no-store" });
